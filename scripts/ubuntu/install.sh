...
if [ "$arch" = "armv7l" ]; then
	...
	cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/usr/local -D BUILD_SHARED_LIBS=ON -D WITH_CUFFT=ON -D WITH_EIGEN=ON -D WITH_OPENGL=ON -D WITH_QT=ON -D WITH_TBB=ON -D BUILD_JPEG=ON -D BUILD_EXAMPLES=ON -D BUILD_TESTS=ON .. >> $logfile 2>&1
fi
